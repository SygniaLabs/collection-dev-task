...
def main():
    ...
    # Cost estimate
    if throughput > 0:
        msgs_per_day = throughput * 86400
        sqs_cost_individual = (msgs_per_day / 1_000_000) * 0.40
        sqs_cost_batched = (msgs_per_day / 10 / 1_000_000) * 0.40
        print("  💰 Projected AWS costs at this throughput (running 24/7):")
        print(f"     SQS (1 msg per request):    ~${sqs_cost_individual:>8,.2f}/day")
        print(f"     SQS (batched, 10/request):  ~${sqs_cost_batched:>8,.2f}/day")
        print(f"     (SQS charges $0.40 per million requests)")
    print("=" * 70)
